...
bot.start((ctx) =>  {
    ctx.reply(`👋 Привет, ${ctx.message.from.first_name}!\n\n🤖 Нажимай на кнопку(если они скрыты можено открыть возле поля ввода или используй /help) и я помогу тебе сгенерировать рандомный фильм/сериал/тв программу!\n\n💬  /help - команда поможет тебе разобраться как работать с ботом`, Markup.keyboard([['Поиск фильма'], ['Поиск сериала/программы'], ['Помощь']]).resize().extra())
    console.log(`-------------->>>> User id: ${ctx.message.from.id}; Username: ${ctx.message.from.username}; User first name: ${ctx.message.from.first_name}; User last name: ${ctx.message.from.last_name}`)
})
...
bot.on('text', async (ctx) => {
    ...
    if(textCapitalize === 'Поиск фильма') {
        ...
        console.log(`------------>>> User ${ctx.message.from.first_name} start looking movie!`);
    ...
    if(textCapitalize === 'Поиск сериала/программы' || textCapitalize === 'Поиск сериала') {
        ...
        console.log(`------------>>> User ${ctx.message.from.first_name} start looking tv shows!`);
    ...
    if(textCapitalize === 'Помощь') {
        ...
        console.log(`------------>>> User ${ctx.message.from.first_name} need help!`);

        const message = `🤖 Я умный бот который поможет тебе подобрать фильм, сериал или программу на вечер.\n\n📖 Нажимай на определенную кнопку "Поиск фильма" или "Поиск сериала/программы", также можно в ручную написать команду поиска и я смогу подобрать фильм/сериал/программу.\n\n💬 "Поиск фильма" - команда поиска фильма\n\n💬 "Поиск сериала/программы" - команда поиска ТВ шоу\n\n💬 "Поиск сериала" - команда поиска ТВ шоу(равнозначна команде "Поиск сериала/программы")\n\n💬 /help - помощь работы с Ботом`
        ctx.reply(message)
    }
    if(textCapitalize !== 'Помощь' && textCapitalize !== 'Поиск сериала/программы' && textCapitalize !== 'Поиск сериала' && textCapitalize !== 'Поиск фильма') {
        ctx.reply(`Мы не можем найти такую команду, попробуйте обратиться в /help .`)
    }
